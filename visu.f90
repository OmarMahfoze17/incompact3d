...
!############################################################################
!
subroutine PROBE (ux1,uy1,uz1,jp,kp)
!
!############################################################################
...
integer :: code,icomplet,jp,kp,jj,kk
integer :: ijk,nvect1,nvect2,nvect3,i,j,k
character(len=20) nfichier,nfichier1
character(len=20) :: filename

953 format('sondes',I4.4)


!print *,nrank, ' cores to',xstart(2),xend(2),xstart(3),xend(3),jp,kp

if (jp .ge.xstart(2).and.jp.le.xend(2).and.kp.ge.xstart(3).and.kp.le.xend(3)) then
print *,nrank, ' cores to',xstart(2),xend(2),xstart(3),xend(3),jp,kp,xsize(1)
!if ((nrank==0)) then
if (itime==ifirst) then
write(filename, 953) nrank
open(nrank,file=filename,form='unformatted')
endif
write(nrank) (((ux1(i,j,k),i=1,xsize(1),8),j=xstart(2)-jp+1,xstart(2)-jp+1),&
              k=xstart(3)-kp+1,xstart(3)-kp+1),&
             (((uy1(i,j,k),i=1,xsize(1),8),j=xstart(2)-jp+1,xstart(2)-jp+1),&
              k=xstart(3)-kp+1,xstart(3)-kp+1),&
             (((uz1(i,j,k),i=1,xsize(1),8),j=xstart(2)-jp+1,xstart(2)-jp+1),&
              k=xstart(3)-kp+1,xstart(3)-kp+1)
...
subroutine VISU_INSTA (ux1,uy1,uz1,phi1,ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1,&
     ...
     ta3,tb3,tc3,td3,te3,tf3,tg3,th3,ti3,di3,phG,uvisu,FY_P,FY_N,FZ_P,FZ_N)   !!! OMAR
!
!############################################################################
...
!           1,uz1,filename)
!************** oamr ******************************  
call fine_to_coarseV(1,FZ_P,uvisu)  !!!!!!! omar
997 format('FZ_P',I3.3)!!!!!!! omar
      write(filename, 997) 1 !!!!!!! omar
call decomp_2d_write_one(1,uvisu,filename,2)!!!!!!! omar
!-----------------------------------------------------------------
call fine_to_coarseV(1,FY_P,uvisu)!!!!!!! omar
998 format('FY_P',I3.3)!!!!!!! omar
      write(filename, 998) 1!!!!!!! omar
call decomp_2d_write_one(1,uvisu,filename,2) !!!!!!! omar
!-----------------------------------------------------------------
call fine_to_coarseV(1,FZ_N,uvisu)  !!!!!!! omar
999 format('FZ_N',I3.3)!!!!!!! omar
      write(filename, 999) 1!!!!!!! omar
call decomp_2d_write_one(1,uvisu,filename,2)!!!!!!! omar
!-----------------------------------------------------------------
call fine_to_coarseV(1,FY_N,uvisu)!!!!!!! omar
1000 format('FY_N',I3.3)!!!!!!! omar
      write(filename, 1000) 1!!!!!!! omar
call decomp_2d_write_one(1,uvisu,filename,2) !!!!!!! omar
!************** oamr ******************************   
!############################################################################
...
real(mytype),dimension(xszS(1),xszS(2),xszS(3)) :: vort_mean,vort_2_mean,tmean
integer :: ijk,nvect1,nvect2,nvect3,i,j,k
integer :: code,icomplet
...
!! *************************************** FRCE FILE (OAMR) *****************************
    write(nfil,*)'            <Attribute Name="FZ_P" Center="Node">'
    write(nfil,*)'               <DataItem Format="Binary" '
    write(nfil,*)'                DataType="Float" Precision="8" Endian="little"'
    write(nfil,*)'                Dimensions="',nz,ny,nx,'">'
    write(nfil,*)'                  FZ_P'//chits
    write(nfil,*)'               </DataItem>'
    write(nfil,*)'            </Attribute>'

    write(nfil,*)'            <Attribute Name="FY_P" Center="Node">'
    write(nfil,*)'               <DataItem Format="Binary" '
    write(nfil,*)'                DataType="Float" Precision="8" Endian="little"'
    write(nfil,*)'                Dimensions="',nz,ny,nx,'">'
    write(nfil,*)'                  FY_P'//chits
    write(nfil,*)'               </DataItem>'
    write(nfil,*)'            </Attribute>'

    write(nfil,*)'            <Attribute Name="FZ_N" Center="Node">'
    write(nfil,*)'               <DataItem Format="Binary" '
    write(nfil,*)'                DataType="Float" Precision="8" Endian="little"'
    write(nfil,*)'                Dimensions="',nz,ny,nx,'">'
    write(nfil,*)'                  FZ_N'//chits
    write(nfil,*)'               </DataItem>'
    write(nfil,*)'            </Attribute>'

    write(nfil,*)'            <Attribute Name="FY_N" Center="Node">'
    write(nfil,*)'               <DataItem Format="Binary" '
    write(nfil,*)'                DataType="Float" Precision="8" Endian="little"'
    write(nfil,*)'                Dimensions="',nz,ny,nx,'">'
    write(nfil,*)'                  FY_N'//chits
    write(nfil,*)'               </DataItem>'
    write(nfil,*)'            </Attribute>'
